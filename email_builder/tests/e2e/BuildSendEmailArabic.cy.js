...
describe('Build and send email', () => {
  ...
  it('Post user data to Mock API Lambda to Build and Send register email template in Arabic', () => {
    const user = {
      templateId: 'register',
      data: {
        name: 'Cypress Test',
    ...
    cy.request('GET', 'http://localhost:4000/aws/cms')
      ...
      .then(function (response) {
        expect(response).property('HEADER').to.equal('اخر الاخبار');
        cy.title().should('eq', this.cmsdata.TITLE);
